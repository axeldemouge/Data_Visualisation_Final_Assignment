...
def star_chart(df, key1, key2):
    ...
    df_countries_filtered = df_countries_filtered[["country_bean_origin", "salt", "lecithin", "sugar", "vanilla",
                                                   "cocoa_butter", "sweetener_other"]].groupby("country_bean_origin").mean()
    ...
    df_continents_filtered = df_continents_filtered[["continent", "salt", "lecithin", "sugar", "vanilla", "cocoa_butter",
    ...
    fig.update_layout(
      title = dict(text = f'<b>Average Ingredient Usage</b> for Bars Made With Beans From <b>{country_1}</b> and <b>{country_2}</b>', xref = 'container', x = 0.5),
      polar = dict(
        radialaxis = dict(
          visible  = True,
          range    = [0, 100]
